...
def draw_card(surf, card, x, y, selected=False, small=False):
    ...
    rank_s = font.render(card["rank"], True, color)
    suit_s = small_font.render(card["suit"], True, color)
    surf.blit(rank_s, (x+4, y+2))
    surf.blit(suit_s, (x+4, y+20))
    # Big center suit
    center_s = big_font.render(card["suit"], True, color)
    surf.blit(center_s, (x + CARD_W//2 - center_s.get_width()//2,
                          y + CARD_H//2 - center_s.get_height()//2))
    # Bottom right (upside down)
    rank_s2 = font.render(card["rank"], True, color)
    suit_s2 = small_font.render(card["suit"], True, color)
    surf.blit(rank_s2, (x + CARD_W - rank_s2.get_width() - 4, y + CARD_H - 36))
    surf.blit(suit_s2, (x + CARD_W - suit_s2.get_width() - 4, y + CARD_H - 20))
